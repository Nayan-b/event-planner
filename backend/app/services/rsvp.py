...
async def create_rsvp(rsvp_data: dict, user_id: str):
    db = next(get_db())
    # Check if RSVP already exists
    existing = await get_user_rsvp_for_event(rsvp_data["event_id"], user_id)
    if existing:
        raise HTTPException(
...
async def update_rsvp(
    rsvp_id: str, rsvp_data: dict, user_id: str
):
    db = next(get_db())
    ...
    if existing["user_id"] != user_id:
        raise HTTPException(status_code=403, detail="Not authorized to update this RSVP")
    
    update_data = {k: v for k, v in rsvp_data.items() if v is not None}
    if not update_data:
        return existing
...
async def get_rsvp(rsvp_id: str):
    db = next(get_db())
    result = db.table("rsvps").select("*").eq("id", rsvp_id).single().execute()
